...
function useForge(){
  ...
  const load=useCallback(async()=>{if(init.current)return;init.current=true;const ver=await S.get("forge:ver");if(!ver){await S.set("forge:ver",16);const cls=[SEED_CLIENT_PAT,SEED_CLIENT_RACHEL,SEED_CLIENT_ANGELA,SEED_CLIENT_ADAM,SEED_CLIENT_DEANNA,SEED_CLIENT_KERIE];await S.set("forge:clients",cls);await S.set("forge:w:pat",SEED_PAT);await S.set("forge:w:rachel",SEED_RACHEL);await S.set("forge:w:angela",SEED_ANGELA);await S.set("forge:w:adam",SEED_ADAM);await S.set("forge:w:deanna",SEED_DEANNA);await S.set("forge:w:kerie",SEED_KERIE)}
    // Migrate: patch scheduleDays + add missing Pat Feb workouts + add Kerie
    if(ver&&ver<16){await S.set("forge:ver",16);const seedMap={pat:SEED_CLIENT_PAT,rachel:SEED_CLIENT_RACHEL,angela:SEED_CLIENT_ANGELA,adam:SEED_CLIENT_ADAM,deanna:SEED_CLIENT_DEANNA};let cls=await S.get("forge:clients");if(cls){cls=cls.map(c=>{const seed=seedMap[c.id];if(seed&&!c.scheduleDays){return{...c,scheduleDays:seed.scheduleDays||[],scheduleNotes:seed.scheduleNotes||c.schedulePattern||""}}return c});
    ...
    let cls=await S.get("forge:clients");if(!cls||!cls.length){const d=[SEED_CLIENT_PAT,SEED_CLIENT_RACHEL,SEED_CLIENT_ANGELA,SEED_CLIENT_ADAM,SEED_CLIENT_DEANNA,SEED_CLIENT_KERIE];cls=d;await S.set("forge:clients",d);await S.set("forge:w:pat",SEED_PAT);await S.set("forge:w:rachel",SEED_RACHEL);await S.set("forge:w:angela",SEED_ANGELA);await S.set("forge:w:adam",SEED_ADAM);await S.set("forge:w:deanna",SEED_DEANNA);await S.set("forge:w:kerie",SEED_KERIE)}setCl(cls);const wm={};for(const c of cls){wm[c.id]=await S.get(`forge:w:${c.id}`)||[]}setWs(wm);setL(false)},[]);
  useEffect(()=>{load()},[load]);
